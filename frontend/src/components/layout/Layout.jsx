...
const Layout = () => {
  ...
  const isStudentShell = role === 'STUDENT'
  ...
  return (
    <div className="flex h-screen bg-gray-950 text-white">
      {/* Left rail: admin / teacher / parent / unlinked keep existing sidebar.
          Students get Instagram-style bottom tabs instead (no cryptic two-letter rail). */}
      {!isStudentShell && (
        <aside className={`
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="bg-gray-900 border-b border-gray-800 px-4 sm:px-6 py-3 sm:py-4
          ...
          {isStudentShell ? (
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <span className="font-bold text-blue-400 text-base truncate">EduMind</span>
            </div>
          ) : (
            <button
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <span className="text-xs sm:text-sm text-gray-400 truncate max-w-[40vw] sm:max-w-none">
              {isAuthenticated ? user.email : 'Demo mode'}
            </span>
            {isAuthenticated && role && (
              <span className="rounded-full bg-blue-500/10 px-2 py-1 text-[10px] sm:text-xs
              font-semibold text-blue-300 shrink-0">
                {role}
              </span>
            )}
            {isStudentShell && isAuthenticated && (
              <button
                type="button"
                onClick={handleLogout}
        ...
        {isStudentShell && <StudentBottomNav />}
      </div>
    </div>
  )
}
